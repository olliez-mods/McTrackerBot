...
def update_server_logs_thread(database: str, uuid:str, ip:str,  port:int):

    # Create SQL connection and curser
    connection = sqlite3.connect(database)
    cursor = connection.cursor()

    if(not table_exists(cursor, f"LOGS_{uuid}")):
        print("New unlogged server... creating TABLE")
        create_logs_table(cursor, uuid)
    ...
    if(make_log):
        ...
        add_record(connection, cursor, uuid, timestamp, 0, status['connected'], status['online'], status['count'], status['players'])
...
def start(sql_database: str, timeout: int=10):
    ...
    while(True):
        ...
        for mc_server in rows:
            ...
            t = threading.Thread(target=update_server_logs_thread, args=(sql_database, uuid, ip, port), daemon=True)
            t.start()
